...
# Vercel + Railway OAuth grant access to ALL projects in the account, so a stray `vercel`/`railway`
# command could touch an unrelated project's prod. This PreToolUse hook (on Bash) restricts any
# vercel/railway invocation to the rasifiters allow-list. Exit 2 = block (stderr shown to the model).
#
...
#   backend: Railway project/service `rasifiters-api`
#
# Until provisioning, this hook BLOCKS any vercel/railway command that names a non-rasifiters
...
# Only police vercel/railway commands.
printf '%s' "$cmd" | grep -qiE '(^|[^[:alnum:]])(vercel|railway)([^[:alnum:]]|$)' || exit 0
