---
name: git-version
description: rasifiters-master's OWN git skill — stage, detect touched feature(s), propose a semver bump, emit the blast-radius report, update registry.json + REGISTRY.md + the SPEC §11 Changelog, commit, and tag feature/<feature>@<version>. This is the repo's git skill — use it for every commit here. Trigger: git/commit/version/bump in rasifiters-master. LIVING — append LESSONS_ARCHIVE.md every run.
---
...
- SPECs are single-file today; the changelog lives in the SPEC's `## 12. Changelog` section — **no
  separate `CHANGELOG.md`** until the feature contract splits (then migrate §11 → CHANGELOG.md).
...
4. **Blast-radius report** (FYI now, hard-gate on contract change):
   ...
   - **Doc blast-radius (volatile-doc prune):** if the commit DELETES a dated/strikethrough log entry
     ...
     resolved fact lives in its canonical home — feature SPEC §11 + registry / an R-entry in
...
- **Uniform owned-interface change; backward-compat → no propagation:** an owned-interface change
  ...
  never `replace_all` a SPEC that mixes current-state with a dated §11 changelog — target the current-state lines.
...
- **Tags invariant:** after each run, every feature's **current** `version` in `registry.json` must have a
  ...
  (57 tags / 16 features as of 2026-07-28) and a count comparison always looks broken. It is also NOT a
  "points at the newest commit" match: read it as *the current version HAS a matching tag*. **Re-tag only
  when the invariant is genuinely broken** (missing / malformed / version-mismatched) — never merely because
  a later commit touched the SPEC. This repo is **public**, so moving a tag means `push --force` and destroys
  the accurate anchor for when that version landed.
- **Commit shapes:** (a) *progress* `feat` (ports code, defers the status flip); (b) *flip* (status
  ...
  (c) *contract change* → real semver bump + §11 Changelog; (d) *faithful asset/format refinement* on a
  deployed feature (e.g. logo PNG→SVG, no contract/behavior delta) → **PATCH** + §11 — the shipped impl
...
- **Volatile-doc log hygiene (R12):** durable docs (SPEC §11, METHODOLOGY log, LESSONS_ARCHIVE) are
  append-only; volatile docs (`PROGRESS.md` "Open items" + "Next action") prune-on-resolve — strike + `DONE`, delete next pass.
