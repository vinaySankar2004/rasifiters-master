...
/// A queued first-sync confirmation for ONE Apple Health flow (workouts OR sleep), one page per
/// program. Built by the compute half of a sync (see `ProgramContext+HealthKit` / `+HealthKitSleep`)
...
struct PendingSyncConfirmation: Identifiable {
    enum Flow: String { case workouts, sleep }
