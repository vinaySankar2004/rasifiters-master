...
> **Status:** 🚀 deployed (live on Render) · **Version:** 0.7.0 · **Apps (`consumed_by`):** `web`, `ios`, `android`
...
| 13 | `POST /oauth` | **NET-NEW** (no legacy handler) → `socialSignIn` (`authService.js`) | no | **Federated (Google/Apple) sign-in.** Exchanges a client-obtained provider `id_token` via Supabase `signInWithIdToken` (R1 — the client never embeds Supabase). Returns a login `AuthResponse` (existing member, by `auth_user_id` else verified provider email → link if unlinked) **or** `{ needs_profile:true, ... }` for a brand-new social user (→ `/oauth/complete`). **409** if the provider email already belongs to a different auth user. **D-C8.** |
