...
> **Status:** 🚀 deployed (ported to `apps/backend/`) · **Version:** 0.2.1 · **Apps (`consumed_by`):** `web`, `ios`, `android`
...
- **Consumers:** **`web` + `ios` + `android`** — the CRUD quartet used 1:1 by all three clients;
  **`POST /batch` is the unified daily-health write path** for the batched multi-row "Log daily health"
  form on all three (0.2.0, D-C5). The single `POST /` remains for the steps/sleep sync writers (the
  iOS quick-add widget now posts to `POST /batch` too — 2026-07-09, D-BATCH).
  ...
  - **ios:** `POST /batch` — `APIClient+DailyHealth.swift` (`addDailyHealthLogsBatch`), from the rebuilt
    `AddDailyHealthDetailView` **and the quick-add health widget `QuickAddHealthWidgetEntryView.swift`**
    (2026-07-09, D-BATCH — the widget became the in-app multi-row form). `POST /` —
    `APIClient+Health.swift` (`addDailyHealthLog`), now only the sleep/steps sync writers
    (`writeHealthKitSleepLog`/`writeHealthKitStepsLog`, POST→409→PUT). `GET /` —
    ...
    `ProgramContext+WorkoutManagement.swift`, from `MemberHealthDetail` swipe (the widget's batch save
    is atomic — no rollback).
...
| **D-REF** | **Reference impl = legacy `backend`.** ~~`consumed_by = [web, ios]` … no batch route~~ → **superseded (0.2.0): `consumed_by = [web, ios, android]`**, and `POST /batch` (D-C5) is now the unified daily-health write path for the batched form on all three clients (the single `POST /` remains for the sync writers; the iOS quick-add widget now posts to `/batch` too — 2026-07-09, D-BATCH). `GET`'s `limit` still differs per caller (web 10/0, iOS 1000) but the route is identical. | Web sweep (`lib/api/{logs,members}.ts` + summary/members pages) + iOS sweep + Android port (Phases A–J); steps-tracking run 2026-07-09. |
...
| Version | Date | Change |
|---------|------|--------|
| 0.2.1 | 2026-07-09 | **Client-side: loggable-only program selector + iOS quick-add health widget → batch form** (no backend/contract change). Mirrors `workout-logs` 0.5.1. (1) **D-LOGGABLE** — the shared program selector on all three "Log daily health" clients now lists only loggable programs (active + not `admin_only_data_entry`-locked; current always kept), dropping completed/locked rows (were shown disabled). (2) **D-BATCH** — the iOS quick-add health **widget** (`QuickAddHealthWidgetEntryView`) is now the in-app multi-row batch "Log daily health" form (no auto-selected program; custom back → My Programs; **gains the steps field**), posting to `POST /daily-health-logs/batch` instead of looping single `POST /` per program (batch is atomic — no rollback). D-REF + the consumer topology corrected. User request 2026-07-09. Consumers `[web, ios, android]`; dependents read data only (unaffected). Page SPECs: `log-health` (ios 0.2.1, android 0.2.1) + `widget-quick-add-health` (ios 0.2.0). Compiles green: iOS `BuildProject` ✓, android `assembleDebug` ✓, web `tsc` ✓. |
