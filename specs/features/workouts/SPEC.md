...
> **Status:** 🚀 deployed (ported to `apps/backend/`) · **Version:** 0.1.1 · **Apps (`consumed_by`):** `ios`
...
| Version | Date | Change |
|---------|------|--------|
| 0.1.1 | 2026-06-30 | **Library seeded with the Apple Health workout types** (additive) for the new [`apple-health`](../apple-health/SPEC.md) iOS auto-sync — `apps/backend/sql/004_seed_healthkit_workout_types.sql` adds ~65 rows (`ON CONFLICT (name) DO NOTHING`, incl. `Other Workout`); no renames, no other-table writes, the existing curated names untouched (D1). No route/service/model change — the library contents grew, the feature code did not. User runs migration `004`. |
