...
| programs | `/programs` (first **protected** route) | 🏗️ v0.2.0 | `programs` v0.2.0 + `program-memberships` + `invites` (CRUD, invites, membership, `saveProgramOrder` `PUT /programs/order` drag-reorder + search) + `auth` (`useAuth`/`useAuthGuard`) | [programs SPEC](web/programs/SPEC.md) |
...
| program-picker | 🏗️ v0.2.0 | `programs` v0.2.0/`invites` via `ProgramContext` (`fetchPrograms` `GET /programs`, `saveProgramOrder` `PUT /programs/order` long-press reorder + `.searchable`, `deleteProgram`, `updateMembershipStatus` `PUT /program-memberships`, `loadPendingInvites`/`loadMembershipDetails`/`loadLookupData`/`persistSession`/`signOut`) | [program-picker SPEC](ios/program-picker/SPEC.md) |
