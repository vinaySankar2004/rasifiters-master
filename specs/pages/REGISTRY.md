...
| members/health | `/members/health` | 🏗️ v0.2.0 | `member-analytics` (`GET /daily-health-logs` list) + `daily-health-logs` 0.2.0 (`PUT`/`DELETE`, steps sort/filter/edit/CSV) + `auth` | [members/health SPEC](web/members/health/SPEC.md) |
...
| member-health-detail | 🏗️ v0.2.0 | `daily-health-logs` 0.2.0 (`GET`/`PUT`/`DELETE` — steps sort/filter/edit/CSV, DC-10 rows) | [member-health-detail SPEC](ios/member-health-detail/SPEC.md) |
...
| member-health-detail | 🏗️ v0.2.0 | `daily-health-logs` 0.2.0 (`GET`/`PUT`/`DELETE /daily-health-logs`; per-member write surface, steps sort/filter/edit/CSV, DC-10 rows) | [member-health-detail SPEC](android/member-health-detail/SPEC.md) |
...
**Inventory: COMPLETE** (Phases A→J shipped, de-scaffolded 2026-07-08; +`lifestyle-steps-timeline` added 2026-07-09 with the steps-tracking run). All 31 Android screen specs are documented above — the 4 bottom tabs (Summary / Members / Lifestyle / Program, admin + standard variants), all their detail drill-downs (incl. the Lifestyle + Steps timelines), the 6 program-settings sub-routes, notifications (settings + real-time alerts), and Health Connect. Widgets (Glance) are deferred (out of v1 scope).
