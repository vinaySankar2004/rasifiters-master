...
|------|-------|--------|---------------------|------|
| landing | `/` (root, **public marketing**) | 🏗️ v0.1.2 | `auth` (foundation `useAuth` for the auth-aware CTA; no API) | [landing SPEC](web/landing/SPEC.md) |
...
| summary/activity | `/summary/activity` (summary **sub-route** 1/6 — chart drill-down) | 🏗️ v0.1.0 | `analytics` (`GET /analytics/timeline`; workouts + active members per bucket, W/M/Y/P) + `auth` | [summary/activity SPEC](web/summary/activity/SPEC.md) |
| summary/distribution | `/summary/distribution` (summary **sub-route** 2/6 — chart drill-down) | 🏗️ v0.1.0 | `analytics` (`GET /analytics/distribution/day`; workouts per weekday, read-only) + `auth` | [summary/distribution SPEC](web/summary/distribution/SPEC.md) |
| summary/workout-types | `/summary/workout-types` (summary **sub-route** 3/6 — chart drill-down) | 🏗️ v0.1.0 | `analytics` (`GET /analytics/workouts/types`; sessions per type + ranked detail list) + `auth` | [summary/workout-types SPEC](web/summary/workout-types/SPEC.md) |
| summary/bulk-log-workout | `/summary/bulk-log-workout` (summary **sub-route** 6/6 — mobile log fallback) | 🏗️ v0.1.0 | `workout-logs` (`POST /workout-logs/batch`) + `program-memberships`/`program-workouts` lookups + `auth` | [summary/bulk-log-workout SPEC](web/summary/bulk-log-workout/SPEC.md) |
| members | `/members` (second **workspace tab**) | 🏗️ v0.3.0 | `member-analytics` 0.4.0 (metrics incl. `avg_steps`/history/streaks/recent) + `daily-health-logs` 0.2.0 (health card, steps) + `program-memberships` (`fetchProgramMembers` → view-as picker) + `auth` (`useAuthGuard`) | [members SPEC](web/members/SPEC.md) |
...
| members/list | `/members/list` (members **sub-route** 1/8) | 🏗️ v0.1.0 | `program-memberships` (`fetchProgramMembers`; searchable read-only active roster, entry to `members/detail`) + `auth` | [members/list SPEC](web/members/list/SPEC.md) |
| members/detail | `/members/detail?memberId=` (members **sub-route** 2/8, **global_admin only**) | 🏗️ v0.1.0 | `program-memberships` (`updateMembership` joined-date/active-flag + `removeMember`) + `auth` | [members/detail SPEC](web/members/detail/SPEC.md) |
| members/invite | `/members/invite` (members **sub-route** 3/8, admin) | 🏗️ v0.1.0 | `invites` (`sendProgramInvite`; `@`-username form + privacy banner) + `auth` | [members/invite SPEC](web/members/invite/SPEC.md) |
| members/metrics | `/members/metrics` (members **sub-route** 4/8, admin entry) | 🏗️ v0.2.0 | `member-analytics` (per-member metric-card grid, search/sort/filter + client CSV export) + `auth` | [members/metrics SPEC](web/members/metrics/SPEC.md) |
| members/history | `/members/history?memberId=&name=` (members **sub-route** 5/8) | 🏗️ v0.1.0 | `member-analytics` (workout-history timeline `BarChart`, W/M/Y/P) + `auth` | [members/history SPEC](web/members/history/SPEC.md) |
| members/streaks | `/members/streaks?memberId=&name=` (members **sub-route** 6/8) | 🏗️ v0.1.0 | `member-analytics` (current/longest streak pills + milestone badges) + `auth` | [members/streaks SPEC](web/members/streaks/SPEC.md) |
| members/workouts | `/members/workouts?memberId=&name=` (members **sub-route** 7/8) | 🏗️ v0.1.0 | `member-analytics` (`GET /member-recent` log list) + `workout-logs` (edit duration / delete rows, CSV) + `program-workouts` (filter dropdown, lazy) + `auth` | [members/workouts SPEC](web/members/workouts/SPEC.md) |
...
| lifestyle/workouts | `/lifestyle/workouts` (lifestyle **sub-route** 1/2) | 🏗️ v0.1.0 | `program-workouts` (all 6 routes: list + hide/show + add/edit/delete custom; admin writes, read-only otherwise) + `auth` | [lifestyle/workouts SPEC](web/lifestyle/workouts/SPEC.md) |
| lifestyle/timeline | `/lifestyle/timeline` (lifestyle **sub-route** 2/2) | 🏗️ v0.1.0 | `analytics` (`GET /analytics/health/timeline`; sleep bars + diet-quality line, W/M/Y/P, read-only) + `auth` | [lifestyle/timeline SPEC](web/lifestyle/timeline/SPEC.md) |
...
| program/profile | `/program/profile` (program-settings **sub-route** 3/6, all roles) | 🏗️ v0.2.0 | `members` (`fetchMemberProfile` `GET /members/:id` + `updateMemberProfile` `PUT /members/:id`, email view/change) + `auth` (`deleteAccount` `DELETE /auth/account`, `useAuthGuard`) | [program/profile SPEC](web/program/profile/SPEC.md) |
| program/password | `/program/password` (program-settings **sub-route** 4/6, all roles) | 🏗️ v0.1.0 | `auth` (`changePassword` `PUT /auth/change-password` + live policy checklist, `useAuthGuard`) | [program/password SPEC](web/program/password/SPEC.md) |
| program/appearance | `/program/appearance` (program-settings **sub-route** 5/6, all roles) | 🏗️ v0.1.0 | none — client-side theme picker (`lib/theme.ts` `rf:appearance` localStorage) + `useAuthGuard`; no API | [program/appearance SPEC](web/program/appearance/SPEC.md) |
| program/privacy | `/program/privacy` (program-settings **sub-route** 6/6, all roles) | 🏗️ v0.2.2 | none — static Privacy Policy content (signed-in twin of public `privacy-policy`) + `useAuthGuard`; no API | [program/privacy SPEC](web/program/privacy/SPEC.md) |
...
**Inventory: COMPLETE** — all 37 on-disk web page SPECs are indexed above (every `specs/pages/web/**/SPEC.md`
has a row; the 17 sub-route rows under `summary`/`members`/`lifestyle`/`program` were back-filled 2026-07-18,
health-check Run 6).
...
| admin-members / standard-members (Members tab) | 🏗️ v0.3.0 | `member-analytics` 0.4.0 (metrics incl. `avg_steps`) + `daily-health-logs` 0.2.0 (health card DC-10 preview) via `ProgramContext` | [admin-members SPEC](ios/admin-members/SPEC.md) |
...
| member-metrics-detail | 🏗️ v0.2.0 | `member-analytics` (`ProgramContext.loadMemberMetrics` `GET /member-metrics`; read-only) — the Member Performance Metrics table | [member-metrics-detail SPEC](ios/member-metrics-detail/SPEC.md) |
...
| members | 🏗️ v0.2.0 | `member-analytics` 0.4.0 (metrics incl. `avg_steps`/history/streaks/recent) + `program-memberships` (`GET /program-memberships/members`) + `daily-health-logs` 0.2.0 (health card, steps) via `ProgramContext` — Tab 2 | [members SPEC](android/members/SPEC.md) |
| member-metrics-detail | 🏗️ v0.2.0 | `member-analytics` (`GET /member-metrics`; search/sort/filter table) | [member-metrics-detail SPEC](android/member-metrics-detail/SPEC.md) |
