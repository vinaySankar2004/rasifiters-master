...
| widgets | 🏗️ v0.1.0 | `workout-logs` 0.5.1 (`POST /workout-logs/batch`) + `daily-health-logs` 0.2.1 (`POST /daily-health-logs/batch`) + `program-memberships`/`program-workouts` lookups — two quick-add Glance home-screen widgets (workout + daily-health) deep-linking `rasifiters://quick-add-*` into the batch forms | [widgets SPEC](android/widgets/SPEC.md) |
