...
|-------|------|------------------------|
| Brand logo | **`BrandMark(size: 64)`** — the real `BrandIcon` asset (D-C1; downsized from 90 for the one-screen fit, v0.2.2). **Was** the legacy placeholder. | legacy `LoginView.swift:128-144`; new `BrandMark.swift` |
...
| **"or" divider + social sign-in** | **NET-NEW (D-C3); restyled to custom dark pills (D-C4, v0.2.1).** An "or" divider, then two identical **`FederatedSignInLabel` dark pills** (input-surface capsule = `AppInputField`'s `systemGray3` hairline, no fill; sized to the primary Login CTA; 18pt logo + 8pt gap + body-semibold label): **"Continue with Google"** (multicolor `GoogleG` asset; `ProgramContext.startGoogleSignIn` → `APIClient.socialSignIn(provider:"google")`) above **"Continue with Apple"** (`apple.logo` SF Symbol; tap → `ProgramContext.startAppleSignIn()` runs our own `ASAuthorizationController` → `socialSignIn(provider:"apple", nonce:, firstName:, lastName:)`). A `needs_profile` response pushes `CreateAccountView` in social mode; else `applyAuthResponse`. 409 email-collision surfaces in the existing `Alert`. | new; see D-C3, D-C4 |
...
| **D-C4** | **Federated buttons restyled to custom dark pills + Apple via a standalone controller (v0.2.1).** Both buttons are now the shared `FederatedSignInLabel` dark pill (input-surface capsule matching `AppInputField` — `systemGray3` hairline, no fill; geometry mirrors the primary Login CTA; centered 18pt logo + 8pt gap + body-semibold primary-label text), matching web: multicolor `GoogleG` asset for Google, `apple.logo` SF Symbol for Apple. **Apple no longer uses the native `SignInWithAppleButton` widget** — a plain `Button` tap calls `ProgramContext.startAppleSignIn()`, which drives our own `ASAuthorizationController` (`AppleSignInCoordinator` now an `NSObject` delegate/presentation-provider that returns `(idToken, rawNonce, fullName)` via an async continuation and retains itself for the flow). The auth request (`.fullName,.email` scopes, `request.nonce = SHA256(rawNonce)`) + response (`socialSignIn`) + `needs_profile`/`applyAuthResponse` handling are unchanged; user-cancel (`ASAuthorizationError.canceled`) stays silent. Still App-Store-4.8-compliant (undistorted Apple mark, "Continue with Apple", high contrast, standard corner radius). | user request (2026-07-10 — web parity for both buttons); web login federated-pill design; `FederatedSignInLabel`, `ProgramContext.startAppleSignIn`, `AppleSignInCoordinator`. |
| **D-DEPS** | **One new dependency — `BrandMark.swift` + the `BrandIcon.imageset`** (shared with splash D-DEPS). v0.2.1 adds the **`GoogleG.imageset`** (multicolor Google "G" SVG, `preserves-vector-representation`) — no new SPM/import. v0.2.0 adds the **GoogleSignIn-iOS SPM package** + the **Sign in with Apple capability** (USER-added in Xcode), `GIDClientID` + a reversed-client-id URL scheme in Info.plist, and `com.apple.developer.applesignin` in the entitlements. v0.1.1: the `forgotPasswordURL` originally added to `APIConfig` was **removed** (the browser hand-off is gone); `APIConfig.supportEmail`/`supportMailtoURL` were added for the native screen's contact fallback, and `APIClient.requestPasswordReset(email:)` for the request call. Every other import was ported in the foundation (run 50). | foundation inventory (run 50); `Config/APIConfig.swift`; `Shared/Services/APIClient+Auth.swift`. |
| **D-KBD** | **Global keyboard dismissal (UX-polish, ALL input screens — not login-specific).** Deliberate net-new dismissal UX applied at both `NavigationStack` roots in `App/AppRootView.swift` (`.dismissKeyboardOnTap()` + `.scrollDismissesKeyboard(.interactively)` + one `.keyboardDoneToolbar()` per root, all via `Shared/Extensions/View+KeyboardDismiss.swift`), plus `.submitLabel(.done)` on the shared `Shared/Components/AppInputField.swift`; documented here as the priority auth surface. Outside-tap resigns first responder; Done/return dismisses. Raw-`TextField` screens (pickers/search/widgets) still get outside-tap but not the Done toolbar (acceptable this pass). **No change to what any field DOES.** Parity with web/android. | `App/AppRootView.swift`; `Shared/Extensions/View+KeyboardDismiss.swift`; `Shared/Components/AppInputField.swift`. |
...
| Version | Date | Change |
|---------|------|--------|
| 0.2.2 | 2026-07-10 | **Auth screens tightened to fit one screen, no scroll (layout only).** After the Google + Apple pills were added (v0.2.1) the Login footer overflowed with dead space. Compressed the vertical rhythm — nothing else changed (no logic, no button behavior, no button styling): `BrandMark` 90→64 (dropped its extra `.padding(.top,10)/.bottom,6)`); the body `VStack` spacing 28→16; heading block spacing 10→8; input-field group spacing 16→14; the social section internal spacing 16→12; the outer `.padding(.top)` 60→24; and the three footer links ("Forgot your password?" / "New here? Create an account" / "Training hard?…" + Privacy Policy) were regrouped into ONE `VStack(spacing: 12)` (was three siblings each separated by the 28pt body gap + a stray `.padding(.top,6)`), so the footer sits close + even. Same compaction applied to `CreateAccountView` (body spacing 20→16, brand 90→64, heading 8→6, top pad 30→20, social 16→12) and `ForgotPasswordView` (spacing 24→16, brand 90→64, top pad 40→24) for consistency; `SplashView` unchanged (its flexible `Spacer()`s already self-balance). iOS compile UNVERIFIED — user builds in Xcode. `apps/ios/.../Features/Auth/{LoginView,CreateAccountView,ForgotPasswordView}.swift`. |
| 0.2.1 | 2026-07-10 | **Federated buttons restyled to custom dark pills + Apple via a standalone controller (D-C4).** Both "Continue with Google" and "Continue with Apple" are now the shared `FederatedSignInLabel` dark pill (input-surface capsule = `AppInputField`'s `systemGray3` hairline with no fill, geometry mirroring the primary Login CTA; centered 18pt logo + 8pt gap + body-semibold primary-label text), matching web — multicolor `GoogleG` asset (new `GoogleG.imageset`) for Google, `apple.logo` SF Symbol for Apple. The native `SignInWithAppleButton` widget is removed; Apple now runs through `ProgramContext.startAppleSignIn()` → our own `ASAuthorizationController` (`AppleSignInCoordinator` is now an `NSObject` delegate + presentation-context provider returning `(idToken, rawNonce, fullName)` via an async continuation, self-retained for the flow). Auth request scopes/nonce (SHA256 on the request, raw to the backend), the `socialSignIn` call, and the `needs_profile`/`applyAuthResponse` routing are all unchanged; user-cancel stays silent. iOS compile is USER-run (Xcode). `apps/ios/.../Features/Auth/LoginView.swift`, `Shared/Models/ProgramContext+Auth.swift`, `Assets.xcassets/GoogleG.imageset` (new). |
