...
> **Stance:** faithful 1:1 port of the legacy iOS `SplashView` **+ ONE web-parity deviation** — the real
> brand icon (D-C1). Oddities flagged §10. **First iOS screen spec.**
...
viewModel.start() }`; the `hasStarted` guard makes it run once.
