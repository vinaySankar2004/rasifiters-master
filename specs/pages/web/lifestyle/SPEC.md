...
| Workout Type Popularity card | A sortable horizontal-bar list with a **count / total-minutes / avg-minutes** segmented toggle and a **top-10 / show-all** switch; bar colors are a hash of the workout name. Rendered **above** the two timeline cards to match iOS/Android card order (DC-10). | lifestyle/page.tsx |
| Lifestyle Timeline card | Clickable → `/lifestyle/timeline?memberId=…`; a Recharts `ComposedChart` (sleep-hours bars + diet-quality line) over the last 10 buckets of the week. | lifestyle/page.tsx |
| Steps Timeline card | `StepsTimelineCard` (clone of `LifestyleTimelineCard`, teal `steps` bar); click → `/lifestyle/steps-timeline`. Last card, matching iOS/Android (DC-10). | lifestyle/page.tsx |
...
| **D-C10** | **Deliberate card reorder → iOS/Android parity.** Card order is now Steps stats → **Workout Type Popularity** → Lifestyle Timeline → Steps Timeline, moving Popularity **above** the two timeline charts. This **supersedes DC-9** (which had kept web's legacy order with Popularity last, "no reorder"). The web layout diverged from the native apps and confused the user; iOS `AdminWorkoutTypesTab.swift:71-91` and Android `LifestyleScreen.kt` both render Popularity above the timelines. Pure JSX reorder — no prop/data/behavior change. | iOS `AdminWorkoutTypesTab.swift:71-91`; user request (match iOS/Android layout). |
...
| Version | Date | Change |
|---------|------|--------|
| 0.2.1 | 2026-07-09 | **Card reorder → iOS/Android parity (DC-10).** Moved the **Workout Type Popularity** card **above** the Lifestyle Timeline + Steps Timeline cards, so the web order (Steps stats → Popularity → Lifestyle Timeline → Steps Timeline) matches iOS `AdminWorkoutTypesTab.swift:71-91` and Android `LifestyleScreen.kt`. Web had rendered Popularity last (the legacy order kept by DC-9), which diverged from the native apps and read as "missing." Pure JSX reorder in `lifestyle/page.tsx` — no prop/data/behavior change; **supersedes DC-9**. |
