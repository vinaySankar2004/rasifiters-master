...
| **D-KBD** | **Global keyboard dismissal (UX-polish, ALL input screens — not login-specific).** Deliberate net-new dismissal UX mounted at the app shell (`apps/web/src/app/shell.tsx` via `lib/hooks/use-dismiss-keyboard.ts`); documented here as the priority auth surface. Capture-phase `pointerdown` blurs the focused `input`/`textarea`/`select`/`contenteditable`/`role=textbox` when the tap lands outside any typing target; `Escape` blurs it. Enter-to-submit already works via native `<form>` semantics (unchanged). **No change to what any input DOES.** Parity with iOS/android (same behavior; native idiom differs). | `apps/web/src/app/shell.tsx`; `apps/web/src/lib/hooks/use-dismiss-keyboard.ts`. |
...
| Version | Date | Change |
|---------|------|--------|
| 0.2.2 | 2026-07-10 | **Tightened auth-screen vertical rhythm to fit a mobile viewport (layout only).** After the Google button was added, the bottom cluster (Forgot password / Create account / Training hard / Privacy) had excessive stacked gaps and the screen overflowed on phone-sized viewports. Compressed evenly: container `pt-14 sm:pt-20`→`pt-10 sm:pt-14`, `pb-12`→`pb-8`; `BrandMark` size `128`→`96`; heading block `mt-8`→`mt-5`; form `mt-10`→`mt-6`; footer cluster "New here?" `mt-6`→`mt-3` and "Training hard?" footer `mt-8`→`mt-5` (Forgot-password `mt-4` unchanged). Same tightening applied to `/create-account` (dots `mt-6`→`mt-4`, form `mt-8`→`mt-6`, "Already have an account?" `mt-6`→`mt-4`, Privacy footer `mt-8`→`mt-5`) for consistency. No auth logic, button styling, or copy changed. Patch bump (layout/spacing only). `npm run build` ✓. |
| 0.2.1 | 2026-07-10 | **Custom dark-pill Google button (auth v0.8.0).** Replaced Google's branded GSI widget with our OWN dark pill (input-surface fill, hairline border, multicolor "G", "Continue with Google") matching the Login CTA. Switched to the GSI **auth-code** flow (`initCodeClient`, popup): the button yields a one-time `code` posted to `/auth/oauth`; the backend exchanges it for an id_token (so the browser never holds the client secret). Same design now on iOS/Android. Visual + flow only; session handling unchanged. `npm run build` ✓. |
