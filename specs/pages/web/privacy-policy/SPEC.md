...
|-------|------|------------------------|
| Header | `PageHeader` "Privacy Policy" + "Effective date: 2026-03-02" subtitle + a **Support** `next/link` action → `/support`. | privacy-policy/page.tsx:11-22 |
| Policy card | `GlassCard padding="lg"` (`space-y-6 text-sm text-rf-text-muted`) wrapping the intro paragraph + the nine titled policy sections. | privacy-policy/page.tsx:24-127 |
| Section | Each section: a `text-base font-semibold text-rf-text` heading + either a paragraph or a `list-disc` `<ul>` of points (collect / use / share / retain / secure / choices / children / changes / contact). | privacy-policy/page.tsx:31-126 |
