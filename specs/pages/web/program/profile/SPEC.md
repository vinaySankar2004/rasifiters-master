...
> gender + Delete Account), the third of the six deferred `/program/*` settings sub-routes (reached from the
...
> iOS port.
> **Stance:** faithful 1:1 port **+ two small cleanups** (D-C1 tokenize the success color; D-C2 clear the stale
> success/error message on field edit). Oddities flagged §10.
...
`<select>`; a **Save changes** button; and — for everyone **except** a global admin — a **Delete Account**
...
| Gender | Native `<select>` — "Select gender" + the 4 `GENDER_OPTIONS`. | [profile/page.tsx:149-163](../../../../../../rasifiters-webapp/src/app/program/profile/page.tsx#L149) |
| Error / success | Inline `rf-danger` error line; **`rf-success`** "Profile updated successfully." (D-C1). | [profile/page.tsx:165-168](../../../../../../rasifiters-webapp/src/app/program/profile/page.tsx#L165) |
| Save | "Save changes" (`bg-rf-accent text-black`); disabled unless both names present + not pending. | [profile/page.tsx:170-181](../../../../../../rasifiters-webapp/src/app/program/profile/page.tsx#L170) |
| Delete Account | **Hidden for global_admin.** Danger-outline button → `ConfirmDialog`. | [profile/page.tsx:183-196](../../../../../../rasifiters-webapp/src/app/program/profile/page.tsx#L183) |

## 5. Components + consumed features

- **Shared UI:** `PageShell`, `PageHeader`, `GlassCard`, `ConfirmDialog` — **all already ported** (no new
  dependency this run). The page-local `GENDER_OPTIONS` + `roleLabel`/`userInitials` derivations stay
  co-located; gender uses a native `<select>` (faithful — not the `Select` component).
...
- **`GET /members/:id`** via `fetchMemberProfile(token, session.user.id)`
  ...
  splits `member_name` into first/last and seeds `gender`.
...
- **No backend work, no feature bump** — all three routes + client fns shipped with `members` (v0.2.0) +
  `auth` (v0.2.0); the cascade was wired when those features landed.
